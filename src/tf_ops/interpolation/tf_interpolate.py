...
def five_kernel(points, tex, idx, radius):
    return interpolate_module.five_kernel(radius=radius, points=points, tex=tex, idx=idx)
@tf.RegisterGradient('FiveKernel')
def _five_kernel_grad(op, grad_out):
    points = op.inputs[0]
    tex = op.inputs[1]
    idx = op.inputs[2]
    return [interpolate_module.five_kernel_grad(radius=op.get_attr('radius'), points=points, tex=tex, idx=idx, grad_out=grad_out), None, None]
