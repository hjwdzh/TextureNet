...
REGISTER_OP("FiveKernel")
    ...
    .Output("out: float32")
    .SetShapeFn([](::tensorflow::shape_inference::InferenceContext* c) {
        ::tensorflow::shape_inference::ShapeHandle dims1, dims2; // (b,m,c)
        c->WithRank(c->input(0), 3, &dims1);
        c->WithRank(c->input(2), 3, &dims2);
        ::tensorflow::shape_inference::ShapeHandle output = c->MakeShape({c->Dim(dims1, 0), c->Dim(dims2, 1), 6, c->Dim(dims1, 2)});
        c->set_output(0, output);
        return Status::OK();
    });
REGISTER_OP("FiveKernelGrad")
    .Attr("radius: float")
    .Input("points: float32")
    .Input("tex: float32")
    .Input("idx: int32")
...
class FiveKernelOp: public OpKernel{
    public:
        explicit FiveKernelOp(OpKernelConstruction * context):OpKernel(context){
            OP_REQUIRES_OK(context, context->GetAttr("radius", &radius_));
        }

        void Compute(OpKernelContext * context) override {
            const Tensor& points_tensor=context->input(0);
            const Tensor& tex_tensor = context->input(1);
            const Tensor& idx_tensor = context->input(2);

            OP_REQUIRES(context, points_tensor.dims()==3, errors::InvalidArgument("FiveKernel expects (b,m,c) points shape"));
            int batch_size = points_tensor.shape().dim_size(0);
            int num_points = points_tensor.shape().dim_size(1);
            int num_featdim = points_tensor.shape().dim_size(2);
            int num_neighbors = idx_tensor.shape().dim_size(2);
            int num_groups = idx_tensor.shape().dim_size(1);

            Tensor * out_tensor = nullptr;
            OP_REQUIRES_OK(context, context->allocate_output(0,TensorShape{batch_size,num_groups,6,num_featdim}, &out_tensor));

            auto points_flat = points_tensor.flat<float>();
            auto tex_flat = tex_tensor.flat<float>();
            auto idx_flat = idx_tensor.flat<int>();
            auto out_flat = out_tensor->flat<float>();
            const float *points = &(points_flat(0));
            const float *tex = &(tex_flat(0));
            const int *idx = &(idx_flat(0));
            float *out = &(out_flat(0));
            cudaMemset(out, 0, sizeof(float) * batch_size * num_points * 6 * num_featdim);
            fivekernel_gpu(batch_size, num_points, num_featdim, num_neighbors, num_groups, points, tex, idx, out, radius_);
        }
        float radius_;
};
REGISTER_KERNEL_BUILDER(Name("FiveKernel").Device(DEVICE_GPU),FiveKernelOp);
...
class FiveKernelGradOp: public OpKernel{
    public:
        explicit FiveKernelGradOp(OpKernelConstruction * context):OpKernel(context){
            OP_REQUIRES_OK(context, context->GetAttr("radius", &radius_));            
        }
        ...
        void Compute(OpKernelContext * context) override {
            ...
            Tensor * grad_points_tensor = nullptr;
            OP_REQUIRES_OK(context, context->allocate_output(0,TensorShape{batch_size,num_points,num_featdim}, &grad_points_tensor));
            ...
            const float *grad_out = &(grad_out_flat(0));
            float *grad_points = &(grad_points_flat(0));
            cudaMemset(grad_points, 0, sizeof(float) * batch_size * num_points * num_featdim);
            fivekernelgrad_gpu(batch_size, num_points, num_featdim, num_neighbors, num_groups, points, tex, idx, grad_out, grad_points, radius_);
        }
        float radius_;
};
REGISTER_KERNEL_BUILDER(Name("FiveKernelGrad").Device(DEVICE_GPU),FiveKernelGradOp);
